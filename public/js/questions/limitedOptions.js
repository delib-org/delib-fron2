    ...
    DB.child("questions/"+questionUid+"/parentEntityUid").once("value", function(dataSnapshot){
      ...
      DB.child("groups").child(parentUid).child("type").once("value", function(typeGroup){
        ...
        if (groupType1 == "public"){
          ...
          //check if member
          DB.child("groups").child(parentUid).child("members").orderByKey().equalTo(userUuid).once("value", function(memberData){

            if (memberData.val() != null){ //if member
              console.log("User "+userUuid+" is a member of group "+ parentUid);
              //show reg footer
